...
Como entrada, foram utilizadas cinco imagens de diferentes tamanhos, conforme apresentado na Tabela \ref{tab.img}. O algoritmo de \textit{shift vector} divide cada imagem em blocos, calcula o vetor de características, ordena os blocos, compara todos os pares de blocos consecutivos, verifica a similaridade por meio de um histograma e descarta os falso-positivos. Observe que, no caso da imagem \textit{img-05}, são gerados 4.971.033 de blocos de informação para o \textit{blocksize} igual a 16x16.
...
Na implementação OpenMP, cada processo recebeu a responsabilidade de calcular as características de um bloco por vez. As cinco imagens foram utilizadas como entrada da aplicação e o número de \textit{threads} utilizadas na detecção foi incrementado em duas unidades a cada interação. Os resultados obtidos podem ser visualizados na Tabela \ref{tab.omp}.
...
O algoritmo paralelizado com OpenMP se mostrou mais eficiente, devido ao fato de não haver a necessidades de tráfego de dados via rede, como é necessário ao MPI. Ainda, o OpenMP
