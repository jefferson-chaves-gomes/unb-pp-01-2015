...
Para as simulações foi utilizado um computador com processador Intel Core i7-4500U CPU @ 1.80GHz (com dois núcleos e suporte a \textit{hyper-threading}) e 8GB de memória RAM. A máquina possui instalado o sistema operacional Ubuntu 14.04 64 bits.
...
O algoritmo de criação do vetor de características possui complexidade igual a $O(n^4)$. Esta é a tarefa mais cara, chegando a consumir 90\% do esforço de processamento da aplicação. A tarefa de ordenação de blocos possui complexidade igual a $O(n lg_2 n)$; a criação de histograma e eliminação de falso-positivos possuem complexidade linear e apresentam dependência de dados entre iterações sucessivas.
...
Tanto no OpenMP quanto no MPI foi possível perceber a degradação da eficiência conforme eram adicionados novos processos em todos os casos de simulação. Isto é resultado da limitação do hardware e demonstra que o problema da paralelização do algoritmo de detecção de \textit{copy-move forgery} não é escalável.
